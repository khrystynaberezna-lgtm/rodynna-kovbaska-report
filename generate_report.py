#!/usr/bin/env python3
"""Generate РОДИННА КОВБАСКА report (index.html) from Databricks.

Скелет на основі Hop Hey QBR. Тягне з Databricks:
  - Monthly metrics (з DATA_START, виключно завершені місяці)
  - Last 4 full weeks
  - Фінансові + операційні KPI, refunds, campaigns, acceptance/availability, top stores

Параметри звіту (нижче в коді):
  PARTNER_NAME    = "RODYNNA KOVBASKA"   # точне значення dim_provider_v2.group_name
  PARTNER_DISPLAY = "РОДИННА КОВБАСКА"   # людська назва для логів, заголовка і JSON
  DATA_START      = "2025-01-01"         # стартова дата monthly даних (YYYY-MM-DD)
"""
...
NETWORK_SUMMARY = f"""
SELECT
  (SELECT COUNT(*) FROM main.ng_delivery.dim_provider_v2
